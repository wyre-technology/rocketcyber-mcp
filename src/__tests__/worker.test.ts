...
describe("Cloudflare Worker entrypoint", () => {
  ...
  it("handles MCP initialize", async () => {
    ...
    expect(body.result?.serverInfo?.name).toBe("rocketcyber-mcp");
  });
  ...
  it("lists all tools without credentials", async () => {
    ...
    expect(names).toContain("rocketcyber_get_account");
    expect(names).toContain("rocketcyber_list_agents");
  ...
  it("returns a graceful error for a credential-requiring tool when unconfigured", async () => {
    const res = await mcp({
      ...
      id: 3,
      method: "tools/call",
      params: { name: "rocketcyber_get_account", arguments: {} },
    });
    expect(res.status).toBe(200);
    ...
    expect(body.result?.content?.[0]?.text).toMatch(/api[_ ]?key|credentials/i);
  });
